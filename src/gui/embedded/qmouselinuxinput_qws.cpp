    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (data->type == EV_ABS) { //EV_ABS == 3
            ...
            if (data->type == 0x3) {
                if (data->code == 0x36) {
                    m_y = data->value;
                } else if (data->code == 0x35) {
                    m_x = data->value;
                } else if (data->code == 0x39) {

                } else if (data->code == ABS_X) {
                    m_x = data->value;
                } else if (data->code == ABS_Y) {
                    m_y = data->value;
                }else {
                    unknown = true;//ABS_PRESSURE:  android only have
                }
            }
...
#if 0
        if (unknown) {
            qWarning("unknown mouse event type=%x, code=%x, value=%d", data->type, data->code, data->value);
        }
#endif
    }
}
